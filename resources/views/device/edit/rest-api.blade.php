...
@section('content')
    <x-device.page :device="$device">
        <x-device.edit-tabs :device="$device" tab="rest-api" />
                    ...
                    <div class="panel-body">
                        ...
                        @else
                            @foreach($device->restApiConnections as $connection)
                                <div class="panel panel-default">
                                    <div class="panel-heading">
                                        <h4 class="panel-title pull-left">{{ $connection->name }}</h4>
                                        <div class="pull-right">
                                            <form action="{{ route('device.rest-api.connections.destroy', [$device, $connection]) }}" method="POST" style="display: inline;">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="btn btn-xs btn-danger" onclick="return confirm('Are you sure you want to delete this connection and all its endpoints?')">Delete Connection</button>
                                            </form>
                                        </div>
                                    ...
                                    <div class="panel-body">
                                        ...
                                        <h5 class="margin-top">Endpoints:</h5>
