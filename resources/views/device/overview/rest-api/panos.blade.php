    ...
    <div class="col-md-12">
        <div class="panel panel-default panel-condensed">
            <div class="panel-heading">
                <i class="fa fa-shield fa-lg icon-theme"></i> <strong>Firewall System Information</strong>
            </div>
            <div class="panel-body">
                <div class="row">
                    <div class="col-md-6">
                        <table class="table table-condensed table-striped">
                            <tr><th class="col-md-6">Hostname</th><td>{{ $system_metrics['hostname']->first()->string_value ?? $device['hostname'] }}</td></tr>
                            <tr><th>Model</th><td>{{ $system_metrics['model']->first()->string_value ?? 'N/A' }}</td></tr>
                            <tr><th>PAN-OS Version</th><td>{{ $system_metrics['sw_version']->first()->string_value ?? 'N/A' }}</td></tr>
                            <tr><th>HA Status</th><td>{{ $system_metrics['ha_state']->first()->string_value ?? 'Standalone' }}</td></tr>
                        </table>
                    </div>
                    <div class="col-md-6">
                        <h4>Session Utilization</h4>
                        {!! print_percentage_bar(400, 40, $session_percent, number_format($session_current) . " / " . number_format($session_max) . " sessions", 'ffffff', $background['left'], $session_max - $session_current, 'ffffff', $background['right']) !!}
                        <p class="text-muted small" style="margin-top: 10px;">
                            <i class="fa fa-info-circle"></i> 
                            Current: {{ number_format($session_current) }} | 
                            Available: {{ number_format($session_max - $session_current) }}
                        </p>
                    </div>
                </div>
            </div>
...
@if($security_policies->count() > 0)
<div class="row">
    <div class="col-md-12">
        <div class="panel panel-default panel-condensed">
            <div class="panel-heading">
                <i class="fa fa-lock fa-lg icon-theme"></i> <strong>Top Security Policies</strong>
                <span class="pull-right text-muted">By hit count</span>
            </div>
            <table class="table table-hover table-condensed table-striped">
                <thead><tr><th>Policy Name</th><th class="text-right">Hit Count</th><th class="text-right">Bytes</th><th class="text-right">Active Sessions</th><th>Last Update</th></tr></thead>
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-right">{{ number_format($policy->sessions ?? 0) }}</td>
                        <td class="text-muted small">{{ \Carbon\Carbon::parse($policy->last_update)->diffForHumans() }}</td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
...
<div class="row">
    ...
    <div class="col-md-6">
        <div class="panel panel-default panel-condensed">
            ...
            <table class="table table-hover table-condensed table-striped">
                <thead><tr><th>Interface</th><th>Status</th><th class="text-right">RX</th><th class="text-right">TX</th></tr></thead>
                <tbody>
    ...
    <div class="col-md-6">
        <div class="panel panel-default panel-condensed">
            <div class="panel-heading">
                <i class="fa fa-bug fa-lg icon-theme"></i> <strong>Threat Statistics</strong>
                <span class="pull-right text-muted">Top threats detected</span>
            </div>
            <table class="table table-hover table-condensed table-striped">
                <thead><tr><th>Threat Type</th><th>Category</th><th class="text-right">Count</th></tr></thead>
                <tbody>
                    @foreach($threats as $threat)
                    <tr>
                        <td>{{ $threat->resource_name }}</td>
                        <td class="text-muted small">{{ $threat->metric_name }}</td>
