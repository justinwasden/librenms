...
class DeviceApiAuthSchemasSeeder extends Seeder
{
    ...
    private function createAuthSchemas(): void
    {
        ...
        // Legacy rename: api_token -> access_token
        ...
        // API Key in fixed header (X-API-Key)
        $apikey = $schema([
            'key' => 'apikey',
            'label' => 'API Key',
        ...
        $upsertField($oauthCC->id, [
            ...
            'required' => false,
            'encrypted' => false,
            'placeholder' => 'space-delimited scopes (optional)',
            'display_order' => 4,
        ]);
        $upsertField($oauthCC->id, [
            'name' => 'audience',
        ...
        $upsertField($oauthAC->id, [
            ...
            'required' => false,
            'encrypted' => false,
            'placeholder' => 'Optional if interactive flow',
            'display_order' => 7,
        ]);
        $upsertField($oauthAC->id, [
            'name' => 'refresh_token',
        ...
        // PureStorage: API token login -> header exchange
        $pureLogin = $schema([
    ...
    {
        // PureStorage FlashArray Template uses the new login schema
        ...
        foreach ($pureEndpoints as $endpoint) {
            DeviceApiTemplateEndpoint::updateOrCreate(
                [
        ...
        $proxmoxTemplate = DeviceApiTemplate::updateOrCreate(
        ...
        foreach ($proxmoxEndpoints as $endpoint) {
            DeviceApiTemplateEndpoint::updateOrCreate(
                [
