...
{
    /**
     * Seed the API templates and auth schemas tables from hardcoded defaults.
     */
    public function run(): void
    {
        $this->seedAuthSchemas();
        $this->seedTemplates();
    }

    /**
     * Seed authentication schemas
     */
    private function seedAuthSchemas(): void
    {
        $authTypes = $this->getAuthSchemas();

        foreach ($authTypes as $key => $schema) {
            ApiAuthSchema::updateOrCreate(
                ['key' => $key],
                [
                    'name' => $schema['name'],
        ...
        $this->command->info('Seeded ' . count($authTypes) . ' auth schemas');
    }
    ...
    private function seedTemplates(): void
    {
        ...
        foreach ($templates as $key => $template) {
            $dbTemplate = ApiTemplate::updateOrCreate(
                ['key' => $key],
                [
                    'name' => $template['name'],
                    'description' => $template['description'] ?? null,
                    'os_types' => $template['os'] ?? [],
                    'auth_type' => $template['auth_type'],
                    'base_url_pattern' => $template['base_url_pattern'] ?? null,
            ...
            foreach ($endpoints as $index => $endpoint) {
                $dbEndpoint = $dbTemplate->endpoints()->updateOrCreate(
                    [
                    ...
                    [
                        ...
                        'for_each' => $endpoint['for_each'] ?? null,
                ...
                $existingEndpointIds[] = $dbEndpoint->id;
            }

            // Remove endpoints that are no longer in the template
            $dbTemplate->endpoints()
                ->whereNotIn('id', $existingEndpointIds)
                ->delete();
        }

        $this->command->info('Seeded ' . count($templates) . ' API templates');

        // Clear the cache so new templates are loaded
        ApiTemplateManager::clearCache();
    }

    /**
     * Get auth schemas to seed
     */
    private function getAuthSchemas(): array
    {
        return [
            'basic' => [
    ...
    private function getTemplates(): array
    {
        return [
            'purestorage_flasharray' => [
                ...
                'capabilities' => ['sensors', 'inventory', 'ports', 'ipv4', 'storage', 'ports_statistics', 'transceivers', 'controllers'],
                'endpoints' => [
                    ...
                    ['capability' => 'inventory', 'method' => 'GET', 'path' => 'controllers',         'transform' => 'LibreNMS\\Util\\Normalizers\\Pure\\Controllers::normalize'],
                    ['capability' => 'controllers', 'method' => 'GET', 'path' => 'hardware?filter=type%3D%27controller%27', 'transform' => 'LibreNMS\\Util\\Normalizers\\Pure\\ControllerHardware::normalize'],
