    ...
    <div class="col-md-12">
        <div class="panel panel-default panel-condensed">
            <div class="panel-heading">
                <i class="fa fa-hdd fa-lg icon-theme"></i> 
                <strong>TrueNAS System Health</strong>
            </div>
            <div class="panel-body">
                <div class="row">
                    <div class="col-md-4">
                        <table class="table table-condensed">
                            ...
                            <tr>
                                ...
                                <td>
                                    <?php 
                                    $uptime = $system_metrics['uptime']->first()->value ?? 0;
                    ...
                    <div class="col-md-4">
                        ...
                        echo print_percentage_bar(300, 40, $mem_percent, Number::formatBi($mem_used) . " / " . Number::formatBi($mem_total), 'ffffff', $mem_bg['left'], $mem_total - $mem_used, 'ffffff', $mem_bg['right']);
                        ?>
                    </div>
                </div>
            </div>
        ...
        <div class="panel panel-default panel-condensed">
            ...
            <table class="table table-hover table-condensed table-striped">
                ...
                <tbody>
                    ...
                    ?>
                    <tr>
                        <td><strong><?php echo htmlspecialchars($pool->resource_name); ?></strong></td>
                        <td>
                            <span class="label label-<?php echo ($pool->status == 'ONLINE') ? 'success' : 'danger'; ?>">
                                <?php echo htmlspecialchars($pool->status ?? 'UNKNOWN'); ?>
                            </span>
                        </td>
                        <td>
                            <span class="label label-<?php echo ($pool->health == 'HEALTHY') ? 'success' : 'warning'; ?>">
                                <?php echo htmlspecialchars($pool->health ?? 'UNKNOWN'); ?>
                            </span>
                        </td>
                        <td><?php echo Number::formatBi($pool->size ?? 0); ?></td>
                        <td><?php echo Number::formatBi($pool->allocated ?? 0); ?></td>
                        <td><?php echo Number::formatBi($pool->free ?? 0); ?></td>
                        <td>
                            <?php
                            echo print_percentage_bar(200, 20, $pool_percent, $pool_percent . "%", 'ffffff', $pool_bg['left'], 100 - $pool_percent, 'ffffff', $pool_bg['right']);
                            ?>
                        </td>
                        <td class="text-right"><?php echo number_format($pool->fragmentation ?? 0); ?>%</td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
...
<div class="row">
    <!-- Top Datasets -->
    <?php if ($datasets->count() > 0): ?>
    <div class="col-md-6">
        <div class="panel panel-default panel-condensed">
            <div class="panel-heading">
                <i class="fa fa-folder fa-lg icon-theme"></i> 
            ...
            <table class="table table-hover table-condensed table-striped">
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="small"><?php echo htmlspecialchars($dataset->resource_name); ?></td>
                        <td>
                            <span class="label label-info">
                                <?php echo strtoupper($dataset->type ?? 'DATASET'); ?>
                            </span>
                        </td>
                        <td class="text-right"><?php echo Number::formatBi($dataset->used ?? 0); ?></td>
                        <td class="text-right"><?php echo number_format($dataset->compression_ratio ?? 1, 2); ?>x</td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
    ...
    <div class="col-md-6">
        <div class="panel panel-default panel-condensed">
            <div class="panel-heading">
                <i class="fa fa-share-alt fa-lg icon-theme"></i> 
                <strong>Network Shares</strong>
                <span class="badge pull-right"><?php echo $shares->count(); ?></span>
            </div>
            <table class="table table-hover table-condensed table-striped">
                <thead>
                    <tr>
                ...
                <tbody>
                    ...
                    <tr>
                        <td><strong><?php echo htmlspecialchars($share->resource_name); ?></strong></td>
                        <td>
                            <span class="label label-primary">
                                ...
                                echo $share_type_map[$share->resource_type] ?? strtoupper($share->share_type ?? 'N/A');
                                ?>
                            </span>
                        </td>
                        <td class="text-muted small"><?php echo htmlspecialchars(substr($share->path ?? '', 0, 40)); ?></td>
                        <td>
                            <span class="label label-<?php echo ($share->enabled == 'true') ? 'success' : 'default'; ?>">
                                <?php echo ($share->enabled == 'true') ? 'ENABLED' : 'DISABLED'; ?>
                            </span>
                        </td>
                    </tr>
                    <?php endforeach; ?>
...
<!-- Replication Tasks -->
<?php if ($replications->count() > 0): ?>
<div class="row">
    <div class="col-md-12">
        <div class="panel panel-default panel-condensed">
            <div class="panel-heading">
                <i class="fa fa-sync fa-lg icon-theme"></i> 
                <strong>Replication Tasks</strong>
            </div>
            <table class="table table-hover table-condensed table-striped">
                <thead>
                    <tr>
                ...
                <tbody>
                    <?php foreach ($replications as $repl): ?>
                    <tr>
                        <td><?php echo htmlspecialchars($repl->resource_name); ?></td>
                        <td>
                            <span class="label label-info">
                                <?php echo strtoupper($repl->direction ?? 'N/A'); ?>
                            </span>
                        </td>
                        <td>
                            <span class="label label-<?php echo ($repl->state == 'FINISHED') ? 'success' : (($repl->state == 'RUNNING') ? 'warning' : 'default'); ?>">
                                <?php echo htmlspecialchars($repl->state ?? 'UNKNOWN'); ?>
                            </span>
                        </td>
                        <td class="text-muted small"><?php echo htmlspecialchars($repl->last_run ?? 'Never'); ?></td>
                        <td class="text-muted small">
                            <?php echo \Carbon\Carbon::parse($repl->last_update)->diffForHumans(); ?>
                        </td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
