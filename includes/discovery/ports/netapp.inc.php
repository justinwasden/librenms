 ...
 *
 * Discovers network interfaces from NetApp via REST API
 */
...
try {
    ...
    \Log::info("NetApp API port discovery found " . count($apiPorts) . " ports", [
        'device_id' => $device['device_id'] ?? 0,
    ]);
} catch (\Throwable $e) {
    \Log::warning("NetApp API port discovery failed: " . $e->getMessage(), [
        'device_id' => $device['device_id'] ?? 0,
    ]);
}
