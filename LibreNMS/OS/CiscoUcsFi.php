...
class CiscoUcsFi extends BaseCisco
{
    ...
    public function discoverOS(\App\Models\Device $device): void
    {
        ...
        try {
            $rows = \SnmpQuery::numeric()->walk('.1.3.6.1.4.1.9.9.719.1.32.1.1')->table(1) ?: [];
            foreach ($rows as $row) {
                if (! is_array($row)) {
                    continue;
                }

                $productName = $row['11'] ?? null;
                $serial = $row['17'] ?? null;
                ...
                if (empty($device->serial) && is_string($serial) && $serial !== '') {
                    $device->serial = $this->sanitizeString($serial);
                }

                if ($hardwareReady && ! empty($device->serial)) {
                    break;
                }
            }
        } catch (\Throwable $e) {
